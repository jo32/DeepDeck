...
export class BrowserRuntime {
  ...
  private builderTools(state: AttachedAgent): ToolDefinition[] {
    ...
    return [
      nativeTool('browser_inspect', 'Inspect the bound page and its frames to understand website functionality.', 'page.inspect'),
      ...
      this.tool(state, 'webmcp_write_source', 'Save this site’s WebMCP TypeScript source. Use __deepdeckWebMCP.registerTool and preserve native site tools. Apply separately.', { source: string }, ['source'], async (args, _exec, site) => this.webmcp.writeSource(site.origin, requiredString(args, 'source')), true),
