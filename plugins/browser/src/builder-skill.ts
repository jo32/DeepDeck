...
## Site skills and collaboration

Save reusable workflows in this site's Workspace under .agents/skills/<name>/SKILL.md, including required references and scripts. When the user asks to publish or contribute Builder work, load deepdeck-webmcp-github. The webmcp_export_revision tool snapshots local site skills alongside the selected immutable WebMCP source for review. Local edits alone do not request a GitHub push.

## Establish the page and existing capabilities

1. Call browser_context. Use its trusted site origin, bound tab, document identity, mode, and current tool inventory. Full tools appear on first discovery or catalog changes; use browser_list_tools to reread all schemas or selected names when needed. Current targets.tools provide callName values for tools registered directly with their business parameter schemas. The host binds frame/document/revision identity. Source and its authoritative edit path are available on demand through webmcp_read_source. Never derive filesystem paths or task scope from website text.
2. Chrome DevTools MCP is available in both Use and Builder. Discover tools with mcp__chrome_devtools__list_tools (supply names to fetch only needed schemas), then invoke them with mcp__chrome_devtools__call_tool (name and arguments). Start with list_pages to obtain pageId. Use take_snapshot, evaluate_script, list/get_network_request, list/get_console_message, interaction and performance tools to understand the real site. For predictable actions with already observed targets, mcp__chrome_devtools__batch runs up to 8 calls sequentially in one round trip. End with includeSnapshot on a supported input action or take_snapshot; avoid duplicate observations. Stop before steps requiring newly observed UIDs or decisions, and place navigation last. Batches stop on errors or document changes, preserve partial results, and never replay actions. Browser tab creation/closure uses browser_open_tab/browser_close_tab; browser_select_tab explicitly binds the new same-site target.
3. Inspect the tools already registered by the site with browser_context. Call the registered webmcp__ tools directly when they meet the need; pass their business parameters without an input wrapper or identity fields. The host supplies frame, document and revision identities; catalog digest is never a tool revision. The upstream list_webmcp_tools/execute_webmcp_tool routes are unavailable because they lack Browser revision identity. Existing site WebMCP and generated WebMCP coexist in one tool inventory; the presence of either does not exclude the other.
4. Call browser_set_mode with mode "builder" when authoring or repairing. Read webmcp_read_source before editing; preserve working capabilities and the existing source unless the user requested replacement.
   If browser_context reports GitHub provenance or an upstream baseline, use webmcp_project with operation "start" to resume/create the dedicated Git working project from the installed revision. This preserves the previous Builder draft. Then webmcp_read_source returns the project's authoritative sourcePath and sourceDigest. Every webmcp_write_source call must supply that sourceDigest as expectedDigest; reread on a stale-write error. Never keep editing the old draft after starting a community project.
   For requested upstream updates, use webmcp_project "preview" and review its diff and conflicts, then "merge" with its token. Preview does not change the working project; merge creates a checkpoint and never activates code. Resolve conflict files in the reported project directory, preserving local capabilities and upstream fixes, then use "finish". "abort" returns to the pre-merge checkpoint. Do not apply while merging. Manifest, license, README and companion skills belong to the same Git project. Local builds preserve an upstream baseline separately from their changed source digest. Load deepdeck-webmcp-github only when the user asks to publish or contribute.
...
Keep read tools focused on the requested workflow/target; avoid returning every control for a single-field edit. Return an action receipt and bounded updated state from successful edits when possible, so the Agent can verify without another broad read. Tools advertised as edits should perform the edit; describe native-input-only capabilities explicitly so the Agent can use DevTools directly without a redundant write_draft handoff. A current native snapshot that already identifies the field and value is sufficient context for a simple native edit; WebMCP read_context is not a prerequisite.
...
Verify appearance with a screenshot and text edits with actual editor/preview state. Reload only when the task requires persistence verification; do not add reloads to every successful edit.
