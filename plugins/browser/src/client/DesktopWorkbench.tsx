...
/** Cordis owns mounting; Better Sidebar owns session tabs, viewers and geometry. */
export function installDesktopWorkbench(ctx: ClientContext, store: SidebarStore, service: BetterSidebarService): void {
  ctx.slots.inject('desktop.workbench', () => ctx.slots.register({ name: 'desktop.workbench' },
    () => <DesktopWorkbench ctx={ctx} store={store} service={service} />))
}

export function DesktopWorkbench({ ctx, store, service }: { ctx: ClientContext; store: SidebarStore; service: BetterSidebarService }) {
