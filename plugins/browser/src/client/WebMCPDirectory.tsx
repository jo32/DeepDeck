...
import { parseCatalog, WEBMCP_REGISTRY_URL, WEBMCP_GUIDE_URL } from '../webmcp-package.ts'
...
export function WebMCPDirectory({ catalog: raw, locale, onInstall, submissionHref = WEBMCP_REGISTRY_URL }: { submissionHref?: string; catalog: unknown; locale: 'en' | 'zh'; onInstall?: ((ref: MarketPackageRef) => void) | undefined }) {
  ...
  return <main className={`${styles.directory} ${market.embedded ? styles.embedded : ''}`}>
    <nav className={styles.navigation} aria-label={zh ? '市场导航' : 'Marketplace navigation'}>
      <div className={styles.navInner}>
        ...
        <div className={styles.navLinks}><a href={zh ? '/webmcp' : '/zh/webmcp'}><Icon name="globe" />{zh ? 'EN' : '中文'}</a><a className={styles.navGithub} href={WEBMCP_GUIDE_URL} target="_blank" rel="noreferrer">GitHub<Icon name="arrow" /></a><a className={styles.primary} href={submissionHref} target="_blank" rel="noreferrer"><Icon name="plus" />{zh ? '提交项目' : 'Submit project'}</a></div>
      </div>
    </nav>
    <div className={styles.shell}>
      <header className={styles.header}>
        ...
        <a className={styles.docsLink} href={WEBMCP_GUIDE_URL} target="_blank" rel="noreferrer"><Icon name="book" />{zh ? '贡献指南' : 'Contribution guide'}<Icon name="arrow" /></a>
      </header>
      <div className={styles.workspace}>
        <aside className={styles.sidebar} aria-label={zh ? '筛选项目' : 'Filter projects'}>
          ...
          <div className={styles.sidebarNote}><Icon name="branch" /><strong>{zh ? '在 GitHub，一起维护。' : 'Better, together.'}</strong><p>{zh ? '遇到过时的工具？向上游提交 Issue 或贡献修复。' : 'Found an outdated tool? Open an issue or contribute a fix upstream.'}</p><a href={WEBMCP_GUIDE_URL} target="_blank" rel="noreferrer">{zh ? '了解协作方式' : 'How to contribute'}<Icon name="arrow" size={14} /></a></div>
        </aside>
        <section className={styles.results} aria-label={zh ? 'WebMCP 项目' : 'WebMCP projects'}>
          ...
          {entries.length === 0 && <section className={styles.empty}>
            ...
            {filtered ? <button className={styles.secondary} onClick={() => { setQuery(''); setFilter('all'); setTag('') }}>{zh ? '清除筛选' : 'Clear filters'}</button> : <a className={styles.primary} href={submissionHref} target="_blank" rel="noreferrer"><Icon name="plus" />{zh ? '发布第一个 WebMCP' : 'Publish your WebMCP'}</a>}
            {!filtered && <a className={styles.emptyLink} href={WEBMCP_GUIDE_URL} target="_blank" rel="noreferrer">{zh ? '查看收录要求' : 'View submission requirements'}<Icon name="arrow" size={13} /></a>}
          </section>}
          <div className={styles.grid}>{entries.map(entry => <article key={entry.id}>
            ...
            <div className={styles.installAction}>{entry.status !== 'active' ? <button className={styles.secondary} disabled>{zh ? '暂不可安装' : 'Unavailable'}</button> : market.embedded ? <button className={styles.primary} onClick={() => { market.install({ repository: entry.repository, manifestPath: entry.manifestPath, repositoryId: entry.repositoryId, ...(entry.commit ? { commit: entry.commit } : {}) }) }}>{zh ? '安装' : 'Install'}</button> : <a className={styles.primary} href={marketInstallLink({ repository: entry.repository, manifestPath: entry.manifestPath, repositoryId: entry.repositoryId, ...(entry.commit ? { commit: entry.commit } : {}) })} onClick={() => { setSelected(entry.id) }}>{zh ? '在 DeepDeck 中安装' : 'Install in DeepDeck'}<Icon name="arrow" size={13} /></a>}</div>
          ...
          <div className={styles.guide}><a href={WEBMCP_GUIDE_URL} target="_blank" rel="noreferrer"><Icon name="code" size={18} /><div><strong>{zh ? '构建并发布' : 'Build & publish'}</strong><p>{zh ? '把网站能力变成可复用工具' : 'Turn site capabilities into reusable tools'}</p></div><Icon name="arrow" size={14} /></a><a href={WEBMCP_GUIDE_URL} target="_blank" rel="noreferrer"><Icon name="branch" size={18} /><div><strong>{zh ? '贡献一份修复' : 'Contribute a fix'}</strong><p>{zh ? '让好用的工具持续可用' : 'Keep useful tools working'}</p></div><Icon name="arrow" size={14} /></a></div>
        </section>
      </div>
