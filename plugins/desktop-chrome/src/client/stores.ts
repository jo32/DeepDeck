...
export function createLayoutStore(): EngineStoreHandle<LayoutState, LayoutActions> {
  return defineStore({
    ...
    actions: {
      ...
      openRightbar: (draft, track, fullscreen) => { draft.details = track ? draft.detailsWidth : 0; draft.rightbarFullscreen = fullscreen },
      ...
      setDetails: (draft, px: number) => {
        draft.detailsWidth = clampWidth(px, DETAILS_MIN, DETAILS_MAX)
        draft.details = draft.detailsWidth
      },
      toggleSidebar: (draft) => {
      ...
      openDetails: (draft) => {
        if (draft.details === 0) draft.details = draft.detailsWidth
      },
