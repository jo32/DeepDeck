...
import css from "./spider-orb.module.css";
import {
  EXPRESSION_OPTIONS,
  type OrbExpression,
...
// Compact diagonal leaves keep a deep black wedge between the alien's eyes.
...
function resolveExpression(expression: Expression, elapsed: number): PoseExpression {
...
function updateEyeGeometry(
  ...
  const writePoint = (
    ...
    const dome =
      bulge *
      Math.pow(Math.max(0, 1 - radialFactor * radialFactor), 1.15);
...
export default function SpiderOrbThree({
  ...
  useEffect(() => {
    ...
    const sphereMaterial = new MeshPhysicalMaterial({
      ...
      roughness: isAlien ? 0.36 : 0.6,
      metalness: 0,
      clearcoat: isAlien ? 0.6 : 0.08,
      clearcoatRoughness: isAlien ? 0.36 : 0.72,
      iridescence: 0,
      iridescenceIOR: 1.32,
    ...
    const eyeGaze = { x: 0, y: 0 };
    const headGaze = { x: 0, y: 0 };
    let gazeBlend = 0;
    ...
    const applyPose = (time: number, snap = false) => {
      ...
      const gazeDestination = gazeTarget.available
        ? gazeTarget
        : { x: 0, y: 0 };
      ...
      headGaze.x += (gazeDestination.x - headGaze.x) * headFollowAmount;
      headGaze.y += (gazeDestination.y - headGaze.y) * headFollowAmount;
      gazeBlend += (gazeBlendTarget - gazeBlend) * gazeBlendAmount;

      const eyeTracking = mapGazeToCharacter(eyeGaze);
      const headTracking = mapGazeToCharacter(headGaze);
      const displayPose = clonePose(currentPose);
      const leftEyeTrackingScale = 0.55 + Math.min(currentPose.left.open, 1) * 0.45;
      const rightEyeTrackingScale = 0.55 + Math.min(currentPose.right.open, 1) * 0.45;
      displayPose.left.x += eyeTracking.eyeX * gazeBlend * leftEyeTrackingScale;
      displayPose.left.y += eyeTracking.eyeY * gazeBlend * leftEyeTrackingScale;
      displayPose.right.x += eyeTracking.eyeX * gazeBlend * rightEyeTrackingScale;
      displayPose.right.y += eyeTracking.eyeY * gazeBlend * rightEyeTrackingScale;
      updateEyePatch(leftEye, displayPose.left);
      updateEyePatch(rightEye, displayPose.right);
      ...
      head.rotation.x =
        ...
        headTracking.headPitch * gazeBlend +
        Math.sin(time * 0.00048 + 1.2) * 0.008 * motion;
      head.rotation.z =
        currentPose.headRoll +
        headTracking.headRoll * gazeBlend +
        Math.sin(time * 0.00038) * 0.009 * motion;
      head.position.y =
        currentPose.lift +
    ...
    const resize = () => {
      ...
      if (gazeTarget.available) {
        ...
        gazeTarget.x = normalized.x;
        gazeTarget.y = normalized.y;
      }
    ...
    const updateGazeTarget = (clientX: number, clientY: number) => {
      ...
      gazeTarget.x = normalized.x;
      gazeTarget.y = normalized.y;
      gazeTarget.clientX = clientX;
      gazeTarget.clientY = clientY;
      gazeTarget.available = true;
      commitGaze();
    };

    const getCharacterHitbox = () => {
      const bounds = container.getBoundingClientRect();
      const size = bounds.width * CHARACTER_HITBOX_WIDTH_RATIO;
    ...
    const finishPointerDrag = (pointerId = drag.pointerId) => {
    ...
    const handlePointerDown = (event: PointerEvent) => {
      ...
      container.dataset.interaction = "drag";
  ...
  return (
    ...
    >
      <div
        className={[
          css.fallback,
          appearance === "whale" ? css.fallbackWhale : undefined,
          appearance === "alien" ? css.fallbackAlien : undefined,
        ]
          .filter(Boolean)
          .join(" ")}
        aria-hidden="true"
      >
        <span
          className={
            appearance === "whale"
              ? [css.whaleEye, css.whaleEyeLeft].filter(Boolean).join(" ")
              : appearance === "alien"
                ? [css.alienEye, css.alienEyeLeft].filter(Boolean).join(" ")
                : [css.fallbackEye, css.fallbackEyeLeft].filter(Boolean).join(" ")
          }
        />
        <span
          className={
            appearance === "whale"
              ? [css.whaleEye, css.whaleEyeRight].filter(Boolean).join(" ")
              : appearance === "alien"
                ? [css.alienEye, css.alienEyeRight].filter(Boolean).join(" ")
                : [css.fallbackEye, css.fallbackEyeRight].filter(Boolean).join(" ")
          }
        />
      </div>
      <span className={css.interactionSurface} aria-hidden="true" />
    </div>
  );
}
