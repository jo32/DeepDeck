...
function shellLifecycle(directory, env) {
  ...
  const cap = async (action, ctx) => {
    try {
      return await run(capsuleBin, [ctx.siteId, action, "--run-id", ctx.runId, "--port", String(ctx.port)], STEP_TIMEOUT_MS[action]);
    } catch (error) {
      // Rejected reservations own no instance; teardown can recreate an empty run
      // directory or touch another invocation. Leave these failures alone.
      if (action === "prepare" && error.stderr?.split("\n").some(line => line === `run already exists: ${ctx.siteId}/${ctx.runId}` || line.startsWith(`port ${ctx.port} is reserved by `) || line === `port is already occupied: ${ctx.port}`)) {
        error.preserveExisting = true;
      }
      // status deliberately exits 1 for absent, stopped, or unhealthy instances.
      if (action === "status" && error.code === 1) {
        let status;
        try { status = JSON.parse(error.stdout); } catch {}
        if (status?.command === "status" && status.site === ctx.siteId && status.runId === ctx.runId && typeof status.healthy === "boolean") return status;
      }
      throw error;
    }
  };
...
export async function bootCapsule(siteId, {
  ...
  const step = async (name) => {
    // Finish the mutator before cleanup: killing just its shell can leave Docker
    // work running and race teardown. Cancellation is checked at safe boundaries.
    if (name !== "down") signal?.throwIfAborted();
    const result = await withTimeout(lifecycle[name](context), limits[name], `${name} for ${siteId}`);
    if (name !== "down") signal?.throwIfAborted();
    return result;
  };
  let stopped = false;
  ...
  let cleanupOnFailure = !reuseExisting;
  try {
    let existing;
    if (reuseExisting) {
      existing = await step("status");
      if (existing?.phase !== "absent") {
        const existingPort = existing?.baseUrl ? Number(new URL(existing.baseUrl).port || 80) : null;
        if (existingPort !== port) throw new Error(`run ${siteId}/${context.runId} already exists at ${existing?.baseUrl ?? "an unknown port"}; use its original port or a different --run-id`);
        if (!healthy(existing) && !["prepared", "ready"].includes(existing.phase)) {
          throw new Error(`run ${siteId}/${context.runId} is ${existing.phase}; inspect it or explicitly run down before recreating it`);
        }
    ...
    const started = existing && healthy(existing) ? existing : await step("up") ?? {};
    ...
    if (cleanupOnFailure && !error.preserveExisting) await down().catch(() => {});
    throw error;
  }
}
