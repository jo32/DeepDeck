...
- Spider：黑色球体和蜘蛛侠眼睛。
- Blue Orb：蓝色球体和两只大圆眼。
- Alien Orb：高光黑色球体、Alien 眼型、暖色面积主光与蓝色背部包裹光。

三种外观共用相同的 Three.js 场景、镜头、液体动画、表情状态机和交互；
Alien Orb 使用自己的材质与灯光方案。

## 预览页面

- `/`：Spider / Blue Orb / Alien Orb 三种并列外观、全表情与复位控制页。
- `/action-orb`：独立的 Alien Orb 操作按钮研究页。正面为 Doing，Send / Stop
  是同一球体背面的球面网格；Doing 状态按 0° → 180° → 360° 循环。紧凑视觉层
  为 48 × 60px，中心仍对应 34px 命中区，向上空间用于三个 Doing 点。

`/action-orb` 只用于设计确认，不会被桌面 app 或 Cordis 插件引用。
