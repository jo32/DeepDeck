...
export function BenchmarkPage({ locale }: { locale: SiteLocale }) {
  ...
  return <div className={s.page}>
    ...
    <header className={s.header}><div className={s.headerInner}>
      ...
      <nav aria-label={t('产品导航', 'Product navigation')}><a href="#capabilities">{t('能测什么', 'What you can test')}</a><a href="#tasks">{t('题库覆盖', 'Task corpus')}</a><a href="#results">{t('实测对比', 'Results')}</a><a href="#pilot">{t('评测交付', 'Deliverables')}</a></nav>
    ...
    <main>
      ...
      <nav className={`${s.pageMap} ${s.shell}`} aria-labelledby="page-map-title">
        ...
        <ol>{[
          ...
          ['results', t('实测结论', 'Measured results'), t('三模型全题库与四组实验', 'Three full-corpus model runs & four experiments')],
      ...
      <section id="results" className={s.resultsBand} aria-labelledby="results-title"><div className={s.shell}><div className={s.sectionHeading}><div><p className={s.eyebrow}>SUCCESS × TOKENS × TIME</p><h2 id="results-title">{t('实测结论', 'Measured results')}</h2></div><p>{t('Terra、DeepSeek v4.1 Flash 与 Hy3 free 完成同一套 49 题。一起看成功率、token 和耗时，也保留超时、评分疑点与环境补跑记录。', 'Terra, DeepSeek v4.1 Flash and Hy3 free completed the same 49-task corpus. Compare pass rates, tokens and time, with timeouts, scoring caveats and environment reruns disclosed.')}</p></div><BenchmarkModels locale={locale} /><details className={s.sampleDisclosure}><summary>{t('其他四组实验：Books、X、HN、OpenAI', 'Four other experiments: Books, X, HN, OpenAI')}<span aria-hidden="true">+</span></summary><BenchmarkComparison locale={locale} /></details><details id="sample" className={s.sampleDisclosure}><summary>{t('评分样例：HN 任务为什么没有完整通过？', 'Scoring example: why did the HN task not fully pass?')}<span aria-hidden="true">+</span></summary><p>{t('一次 CU 运行中，100 条正文正确，但仅 25 条回复关系得到确认。展开检查各项要求与核验依据。', 'In one CU run, all 100 text entries were correct, but only 25 reply relationships were confirmed. Inspect the requirements and evidence below.')}</p><BenchmarkEvidence locale={locale} /></details><a className={s.evidenceLink} href={experiments}>{t('查看完整实验报告与运行条件', 'Read the full experiments and run conditions')} ↗</a></div></section>
      ...
      <div id="apply" className={`${s.application} ${s.shell}`} aria-labelledby="apply-title"><div className={s.applyCopy}><p className={s.eyebrow}>BENCHMARK AGAINST WHAT’S POSSIBLE</p><h2 id="apply-title">{t('想测模型，或验证 WebMCP？', 'Evaluating a model or WebMCP?')}<br />{t('告诉我们要做哪些任务。', 'Tell us which tasks it should do.')}</h2><p>{t('告诉我们模型、目标网站和任务，或者你想验证的 WebMCP。可以比较模型版本，也可以设计开启／关闭工具的消融实验。', 'Tell us your model, target website and tasks, or the WebMCP you want to validate. Compare model versions or design an on/off tool ablation.')}</p><ol><li>{t('确定任务与成功标准', 'Define tasks and success criteria')}</li><li>{t('固定 Harness，选择模型对比或 WebMCP 消融', 'Fix the harness; compare models or ablate WebMCP')}</li><li>{t('比较各版本的结果、Token 和耗时', 'Compare each version’s results, tokens, and time')}</li></ol><a href={experiments}>{t('查看已有实测证据', 'Read the experimental evidence')} ↗</a></div><BenchmarkPilot locale={locale} /></div></section>
    </main>
    <footer className={`${s.footer} ${s.shell}`}><a className={s.brand} href={home}>DeepDeck <b>Bench</b></a><span>GROUND TRUTH · TOKENS · TIME</span><div><a href={experiments}>{t('实验报告', 'Experiments')}</a><a href="https://github.com/jo32/DeepDeck">GitHub ↗</a></div></footer>
  </div>;
}
