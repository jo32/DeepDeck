    ...
    <main>
      ...
      <nav className={`${s.pageMap} ${s.shell}`} aria-labelledby="page-map-title">
        ...
        <ol>{[
          ...
          ['results', t('Sample 评测结论', 'Sample findings'), t('四组实测与评分依据', 'Four experiments and scoring evidence')],
      ...
      <section id="results" className={s.resultsBand} aria-labelledby="results-title"><div className={s.shell}><div className={s.sectionHeading}><div><p className={s.eyebrow}>SUCCESS × TOKENS × TIME</p><h2 id="results-title">{t('Sample 评测结论', 'Sample findings')}</h2></div><p>{t('切换四组已完成实验，比较任务结果、token 和耗时。OpenAI 网站实验的成功结果由用户人工确认；各组的统计口径与比较边界单独说明。', 'Switch between four completed experiments to compare task outcomes, tokens, and time. Success for the OpenAI website experiment was confirmed manually by the user; each experiment states its aggregation and comparison limits.')}</p></div><BenchmarkComparison locale={locale} /><details id="sample" className={s.sampleDisclosure}><summary>{t('评分样例：HN 任务为什么没有完整通过？', 'Scoring example: why did the HN task not fully pass?')}<span aria-hidden="true">+</span></summary><p>{t('一次 CU 运行中，100 条正文正确，但仅 25 条回复关系得到确认。展开检查各项要求与核验依据。', 'In one CU run, all 100 text entries were correct, but only 25 reply relationships were confirmed. Inspect the requirements and evidence below.')}</p><BenchmarkEvidence locale={locale} /></details><a className={s.evidenceLink} href={experiments}>{t('查看完整实验报告与运行条件', 'Read the full experiments and run conditions')} ↗</a></div></section>
