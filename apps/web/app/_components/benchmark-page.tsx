...
export function BenchmarkPage({ locale }: { locale: SiteLocale }) {
  ...
  return <div className={s.page}>
    ...
    <main>
      ...
      <section id="results" className={s.resultsBand} aria-labelledby="results-title"><div className={s.shell}><div className={s.sectionHeading}><div><p className={s.eyebrow}>SUCCESS × TOKENS × TIME</p><h2 id="results-title">{t('实测结论', 'Measured results')}</h2></div><p>{t('Terra、DeepSeek v4.1 Flash 与 Hy3 完成同一套 49 题。一起看成功率、token 和耗时，也保留超时、评分疑点与环境补跑记录。', 'Terra, DeepSeek v4.1 Flash and Hy3 completed the same 49-task corpus. Compare pass rates, tokens and time, with timeouts, scoring caveats and environment reruns disclosed.')}</p></div><BenchmarkModels locale={locale} /><details className={s.sampleDisclosure}><summary>{t('其他四组实验：Books、X、HN、OpenAI', 'Four other experiments: Books, X, HN, OpenAI')}<span aria-hidden="true">+</span></summary><BenchmarkComparison locale={locale} /></details><details id="sample" className={s.sampleDisclosure}><summary>{t('评分样例：HN 任务为什么没有完整通过？', 'Scoring example: why did the HN task not fully pass?')}<span aria-hidden="true">+</span></summary><p>{t('一次 CU 运行中，100 条正文正确，但仅 25 条回复关系得到确认。展开检查各项要求与核验依据。', 'In one CU run, all 100 text entries were correct, but only 25 reply relationships were confirmed. Inspect the requirements and evidence below.')}</p><BenchmarkEvidence locale={locale} /></details><a className={s.evidenceLink} href={experiments}>{t('查看完整实验报告与运行条件', 'Read the full experiments and run conditions')} ↗</a></div></section>
