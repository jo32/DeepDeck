...
export function BenchmarkPilot({ locale }: { locale: SiteLocale }) {
  ...
  return <form className={s.pilotForm} onSubmit={submit}>
    <div className={s.formTop}><span className={s.eyebrow}>YOUR FIRST EVALUATION</span><h3>{t('申请试点评测','Apply for a pilot')}</h3><p>{t('告诉我们要评测的模型，或想通过消融实验验证的 WebMCP，以及目标网站和任务。','Tell us which model to benchmark or which WebMCP to validate through ablation, along with the website and tasks.')}</p></div>
    ...
    <fieldset disabled={state==='sending'} className={s.formFields}>
      ...
      <div className={s.fieldGrid}><label>{t('主要评测目标','Primary goal')}<select name="goal"><option value="compare">{t('比较模型或 Agent 版本','Compare models or agent versions')}</option><option value="regression">{t('检测工作流退步','Detect workflow regressions')}</option><option value="readiness">{t('检验 WebMCP 的实际价值','Evaluate WebMCP’s practical value')}</option></select></label><label>{t('Agent 的网页采集方式','Agent observation surface')}<select name="surface"><option value="screenshots">{t('截图与鼠标键盘','Screenshots, mouse and keyboard')}</option><option value="accessibility">{t('无障碍树与 UI 操作','Accessibility tree and UI actions')}</option><option value="dom">{t('包含 DOM 或浏览器脚本','Includes DOM or browser scripts')}</option><option value="undecided">{t('尚未确定','To be defined')}</option></select></label></div>
      <label>{t('任务与成功标准','Tasks and success criteria')}<textarea name="tasks" required minLength={10} maxLength={2000} rows={4} placeholder={t('例如：比较两个模型的站内搜索能力；或对同一 query 开关 WebMCP，比较答案、token 与耗时。','For example: compare two models on site search, or run the same query with WebMCP on/off to compare answers, tokens, and time.')} /></label>
