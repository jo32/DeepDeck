...
export function BenchmarkUseCases({ locale }: { locale: SiteLocale }) {
  ...
  --n 3`}</code></pre></article>
    ...
    <p className={s.referenceDefinition}>{t('两项能力共享 DeepDeck 的执行环境。这里的 Codex app 式体验指交互工作方式；实际运行基于 DeepSeek Harness 与 DeepDeck 的浏览器工具集，并非 OpenAI Codex app 的同一内部实现，也不代表已证明 SOTA 排名。', 'Both evaluations share DeepDeck’s execution environment. The Codex app comparison describes the interaction workflow. Runs use DeepSeek Harness and DeepDeck’s browser tools, not the same internal implementation as OpenAI’s Codex app, and do not establish a SOTA ranking.')}</p>
