  ...
  return <article className={s.report} aria-labelledby="models-title">
    ...
    <div className={s.detailBody}>
      ...
      <div className={s.tableScroll} tabIndex={0} role="region" aria-label={t('模型结果总览', 'Model results overview')}><table className={s.crossTable}><thead><tr><th scope="col">{t('指标', 'Metric')}</th>{commonTotals.map(item => <th scope="col" key={item.id}>{item.name}</th>)}</tr></thead><tbody>
        <tr><th scope="row">{t('Thinking / 推理强度', 'Thinking / reasoning effort')}<small>{t('两组配置一致', 'Same configuration in both arms')}</small></th>{commonTotals.map(item => <td key={item.id}>{item.reasoning.requestedEffort}<small className={s.absoluteTime}>{t('未显式指定；实际档位未记录', 'Not explicitly set; effective level not recorded')}</small></td>)}</tr>
    ...
    <details className={s.disclosure} open><summary>{t('逐题横向对比 · 三个模型', 'Task comparison · all three models')}<span>49 {t('题', 'tasks')} ＋</span></summary><div className={s.detailBody}>
      <div className={s.taskToolbar}><label className={s.filter}>{t('筛选网站', 'Filter by site')}<select value={site} onChange={event => setSite(event.target.value)}><option value="all">{t('全部网站', 'All sites')}</option>{data.models[0].sites.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select><span>{tasks.length} {t('题', 'tasks')}</span></label><button type="button" className={s.costToggle} aria-expanded={showCosts} aria-controls="task-comparison" onClick={() => setShowCosts(value => !value)}>{showCosts ? t('收起全部费用明细', 'Collapse all cost details') : t('展开全部费用明细', 'Expand all cost details')}</button></div>
      ...
      <div id="task-comparison" className={`${s.tableScroll} ${s.taskScroll}`} tabIndex={0} role="region" aria-label={t('逐题对照表', 'Task comparison table')}><table className={s.crossTable}><thead><tr><th scope="col">{t('题目 / WebMCP', 'Task / WebMCP')}</th>{data.models.map(item => <th scope="col" key={item.id}>{item.name}</th>)}</tr></thead><tbody>{tasks.map(task => <Fragment key={`${task.site}/${task.id}`}>
        ...
        {(['on', 'off'] as const).map(arm => <tr key={arm}><th scope="row"><span className={s.armLabel}>WebMCP {arm.toUpperCase()}</span></th>{data.models.map(item => {
          ...
          </div>}<span className={result.passed ? s.passStatus : s.failStatus}>{result.timedOut ? t('超时', 'Timeout') : result.passed ? t('通过', 'Passed') : t('未通过', 'Failed')}</span></td>;
        })}</tr>)}
    ...
    <details className={s.disclosure}><summary>{t('运行条件与解读边界', 'Run conditions & limitations')}<span>＋</span></summary><div className={s.detailBody}><dl className={s.conditions}>
      ...
      <div><dt>{t('模型与接口', 'Models & interfaces')}</dt><dd>{t('Terra 通过 ChatGPT 登录使用 openai-codex / gpt-5.6-terra；DeepSeek 和 Hy3 通过本地 OpenAI 兼容代理，路由分别为 cn:deepseek-v4.1-flash、cn:hy3，均未显式指定推理强度（报告记录为 provider-default），最终生效的 thinking level 未记录，不能据此认定为关闭、medium 或 high。没有独立验证代理上游身份。接口不同也可能影响耗时。Terra 跨过环境就绪检查修复，包含两个题库版本；下载数据保留版本哈希。这不是同一构建下的严格跨模型排名。', 'Terra uses ChatGPT login via openai-codex / gpt-5.6-terra. DeepSeek and Hy3 use a local OpenAI-compatible proxy with cn:deepseek-v4.1-flash and cn:hy3. No reasoning effort was explicitly requested (recorded as provider-default). The effective thinking level was not retained, so it cannot be classified as off, medium, or high. Proxy upstream identities were not independently verified. Different interfaces can also affect latency. Terra spans two corpus revisions around an environment readiness fix; hashes are retained in the download. This is not a strict same-build cross-model ranking.')}</dd></div>
