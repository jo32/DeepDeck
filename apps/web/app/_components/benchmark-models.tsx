...
export function BenchmarkModels({ locale }: { locale: SiteLocale }) {
  ...
  return <article className={s.report} aria-labelledby="models-title">
    ...
    <details className={s.disclosure}><summary>{t('API 单价、49 题费用合计与计价说明', 'API rates, all-49-task costs & accounting')}<span>2026-09-17 ＋</span></summary><div className={s.detailBody}>
      <p className={s.note}>{t('按公网 API 价估算模型 token 费用，不是本次订阅／代理账单。单价单位为每百万 token，保留原币种（USD 美元、CNY 人民币），不做汇率换算。Hy3 按付费 API 价格估算。', 'Model-token estimates at public API prices, not subscription/proxy bills. Rates are per million tokens in original currencies (USD / CNY), without FX conversion. Hy3 is estimated at paid API rates.')}</p>
